...
def create_iow_lambda_widgets(region, iow_functions):
    """
    Iterate over list of iow lambdas and create generic widgets for each lambda
...
def create_ecosystem_switch_widgets(region, iow_functions):
    """
    Iterate over list of iow lambdas and create generic widgets for ecosystem switch lambdas

    :param iow_functions: filtered list of iow lambda functions
    :param region: The region, for us that's usually us-west-2
    :return: List of ecosystem switch lambda widgets
    :rtype: list
    """
    ...
    ecosystem_lambdas_title_widget = {
        'type': 'text',
        'height': positioning['height'],
        'width': positioning['width'],
        'properties': {
    ...
    ecosystem_switch_widgets.append(ecosystem_lambdas_title_widget)
    ...
    for function in iow_functions:
        ...
        if 'es-logs-plugin' not in function_name and 'aqts-capture-ecosystem-switch' in function_name:
            widget_list = generate_row_of_lambda_widgets(function_name, region, title)
            ecosystem_switch_widgets.extend(widget_list)

    return ecosystem_switch_widgets


def generate_row_of_lambda_widgets(function_name, region, title):
    ...
    return [numeric_stats_widget, concurrent_executions_widget, duration_widget]
